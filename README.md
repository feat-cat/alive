...
- **tools + parameters wrapped (OpenAI JSON Schema):** tools sent to the AI Gateway are wrapped in the OpenAI-standard `{ type: 'function', function: { name, description, parameters } }` shape, and `parameters` is now a complete JSON Schema object (`{ type: 'object', properties, required }`; required = all parameter keys, since tools don't distinguish required/optional). The internal registry stays flat `LlmToolDef`; conversion happens only at send time in `_llm.ts`. The real gateway previously 400'd both the flat shape (`tools[0].type is invalid or missing`) and the bare property-map parameters (`got 'type': null`).
