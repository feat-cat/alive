...
- **tools format fixed (OpenAI function wrapper):** tools sent to the AI Gateway are now wrapped in the OpenAI-standard `{ type: 'function', function: { name, description, parameters } }` shape. The internal registry stays flat `LlmToolDef`; conversion happens only at send time in `_llm.ts`. The real gateway previously 400'd the flat shape (`tools[0].type is invalid or missing`).
